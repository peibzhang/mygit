...
if($action=='send')
{
	...
	{
		$row = $dsql->GetOne("SELECT dtime FROM `sucms_comment` WHERE `ip` = '$ip' ORDER BY `id` desc ");
	...
	$tmpname=$_SESSION['sucms_user_name'];
	if($msg!='')
	{
		$msg = _Replace_Badword($msg);
		$inquery = "INSERT INTO `sucms_comment`(`v_id`,`uid`,`username`,`ip`,`ischeck`,`reply`,`agree`,`anti`,`dtime`,`msg`,`m_type`) VALUES ('$id','$uid','$tmpname','$ip',$ischeck,$cparent,0,0,'$dtime','$msg','$itype'); ";
...
{
	$addagree = "update `sucms_comment` set agree=agree+1 where id=".$id;
	$dsql->ExecuteNoneQuery($addagree);
	$rs = $dsql->GetOne("select v_id,typeid from sucms_comment where id=".$id);
	delfile("../../data/cache/review/".$rs['typeid']."/".$rs['v_id'].".js");
}
elseif($action=='3')
{
	$addagree = "update `sucms_comment` set anti=anti+1 where id=".$id;
	$dsql->ExecuteNoneQuery($addagree);
	$rs = $dsql->GetOne("select v_id,typeid from sucms_comment where id=".$id);
	delfile("../../data/cache/review/".$rs['typeid']."/".$rs['v_id'].".js");
}
