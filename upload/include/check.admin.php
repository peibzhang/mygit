...
$admincachefile = sucms_DATA.'/admin_'.cn_substr(md5($cfg_cookie_encode),24).'.php';
...
class userLogin
{
	...
	var $adminDir = '';
	var $groupid = '';
	var $keepUserIDTag = "sucms_admin_id";
	var $keepgroupidTag = "sucms_group_id";
	var $keepUserNameTag = "sucms_admin_name";
	...
	function checkUser($username,$userpwd)
	{
		...
		$dsql->SetQuery("Select * From `sucms_admin` where name like '".$this->userName."' and state='1' limit 0,1");
		$dsql->Execute();
		$row = $dsql->GetObject();
		...
		else
		{
			...
			$inquery = "update `sucms_admin` set loginip='$loginip',logintime='".time()."' where id='".$row->id."'";
