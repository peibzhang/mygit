...
class Image
{
	...
	function _imgwatermark($filePath,$replace = 1) {
			...
			$filePath = sucms_ROOT.'/'.$filePath;
			$photo_markimg = sucms_ROOT."/data/mark/".$photo_markimg;
	...
	function _strwatermark($filePath){
		    ...
		    $temp = imagettfbbox ( ceil ( $photo_fontsize * 2.5 ), 0 ,sucms_DATA."/mark/simhei.ttf" , $photo_watertext ); //取得使用 TrueType 字体的文本的范围
		    $w = $temp [ 2 ] - $temp [ 6 ];
			$h = $temp [ 3 ] - $temp [ 7 ];
			unset( $temp );     			
			switch($photo_waterpos) {
