...
{
	@session_start();
	return isset($_SESSION['sucms_ckstr']) ? $_SESSION['sucms_ckstr'] : '';
}
...
{
	@session_start();
	$_SESSION['sucms_ckstr'] = '';
	$_SESSION['sucms_ckstr_last'] = '';
}
...
{
	return "<p>Powered by <strong><a href=\"http://www.sucms.org\" title=\"".$GLOBALS['cfg_softname']."\" target=\"_blank\">".$GLOBALS['cfg_soft_enname']."</a></strong> <em>".$GLOBALS['cfg_version']."</em></p>";
}
...
function Html2Text($str,$r=0)
{
	...
	{
		require_once(sucms_INC."/inc/inc_fun_funString.php");
	}
	if($r==0)
	{
...
function CreateDir($spath)
{
	...
	{
		require_once(sucms_INC.'/inc/inc_fun_funAdmin.php');
	}
...
{
	$textsegment=sucms_DATA.'/admin/textsegment.xml';
...
function makePageNumberLoop($currentPage,$pagelistLen,$totalPages,$loopStr){
	...
	for($i=$istart;$i<=$iEnd;$i++)
	{
		...
		$tmp=str_replace("[pagenumber:page]", $i, $loopStr);
		$ret.=$tmp;
	}
	unset($tmp);
	return $ret;
...
function getTopicPageLink($topicId,$page)
{
	...
	if($GLOBALS['cfg_runmode']=='0'){
		$row=$dsql->GetOne("select enname from sucms_topic where id=".$topicId);
...
function setCache($p_cacheName,$sql="",$arr=""){
	...
	$cacheFile=sucms_ROOT.'/data/cache/'.$cfg_cachemark.$p_cacheName.'.inc';
...
function lib_hotwords($num=5)
{
	...
	$dsql->SetQuery("Select keyword From `sucms_search_keywords` where lasttime>$mintime And length(keyword)<$maxlength order by count desc limit 0,$num");
	$dsql->Execute('hw');
	$hotword = '';
	$i=1;
...
function front_member()
{
	...
	$member = '<span id="sucms_member"></span><script>member()</script>';
	}
	return $member;
}
...
function getDataCount($countType){
	...
	switch ($countType) {
		case "all":
		$row = $dsql->GetOne("select count(*) as dd From `sucms_data`");
		$DataCount=$row['dd'];
		break;
		case "day":
		$row = $dsql->GetOne("select count(*) as dd From `sucms_data` $whereStr");
		$DataCount=$row['dd'];
		break;
	}
	return $DataCount;
...
function getTypeLists($type=0)
{
	...
	$sql="select tid,upid,tname,tenname,torder,templist,templist_1,keyword,description,ishidden,unionid,tptype,title,-1 as tcount from sucms_type where tptype = '$type' order by torder asc";
	$rows=array();
	$dsql->SetQuery($sql);
	$dsql->Execute('al');
...
{
	global $dsql,$cfg_iscache;
	$sql="select tid,tname,ishidden,-1 as tcount from sucms_jqtype";
	$rows=array();
	$dsql->SetQuery($sql);
	$dsql->Execute('al');
	while($rowr=$dsql->GetObject('al'))
...
function getNumPerTypeOfNews($id)
{
	...
	$sql = "SELECT count(n_id) as dd FROM `sucms_news` where tid in (".$ids.")";
	$row=$dsql->GetOne($sql);
	if(is_array($row))
	{
		$num=$row['dd'];
...
function getNumPerType($id)
{
	...
	$row=$dsql->GetOne("SELECT count(v_id) as dd FROM `sucms_data` where tid in (".$ids.")");
	if(is_array($row))
	{
		$num=$row['dd'];
	}
...
function getNumPerjqType($id)
{
	...
	$tlist = getTypeListsOnCache();
	$ids = getTypeIdOnCache($id);
	$row=$dsql->GetOne("SELECT count(v_id) as dd FROM `sucms_data` where v_jq like '%$ids%'");
	if(is_array($row))
	{
		$num=$row['dd'];
	}
...
{
	global $dsql;
	$row = $dsql->GetOne("SELECT n_title FROM sucms_news WHERE n_id=$Id");
	return $row['n_title'];
}
...
{
	global $dsql;
	$row = $dsql->GetOne("SELECT n_entitle FROM sucms_news WHERE n_id=$Id");
...
{
	global $dsql;
	$row = $dsql->GetOne("SELECT v_name,v_enname FROM sucms_data WHERE v_id=$videoId");
	if($row['v_enname']!="")
	{
		return $row['v_enname'];
	}
...
{
	global $dsql;
	$row = $dsql->GetOne("SELECT v_name FROM sucms_data WHERE v_id=$videoId");
	return $row['v_name'];
}
...
{
	global $dsql;
	$row = $dsql->GetOne("SELECT v_addtime FROM sucms_data WHERE v_id=$videoId");
...
{
	global $dsql;
	$row = $dsql->GetOne("SELECT n_addtime FROM sucms_news WHERE n_id=$Id");
...
function chkFileCache($cacheName)
{
	...
	$cacheFile=sucms_ROOT.'/data/cache/'.$cfg_cachemark.$cacheName.'.inc';
	$mintime = time() - $cfg_cachetime*60;
	if(!file_exists($cacheFile) || ( file_exists($cacheFile) && ($mintime > filemtime($cacheFile)))){
		return false;
	}else{
...
{
	global $dsql;
	$sql="select name from sucms_topic where id=".$id;
	$row=$dsql->GetOne($sql);
	$tpname=$row['name'];
	return $tpname;
}
...
{
	$labelRule = buildregx("{sucms:".$ptype."list(.*?)size=([0-9]+)(.*?)}","is");
...
{
	$labelRule = buildregx("{sucms:customvideolist(.*?)size=([0-9]+)(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{sucms:customvideolist(.*?)lang=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{sucms:customvideolist(.*?)year=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{sucms:customvideolist(.*?)area=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{sucms:customvideolist(.*?)order=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{sucms:customvideolist(.*?)time=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{sucms:customvideolist(.*?)commend=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{sucms:customvideolist(.*?)type=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{sucms:customvideolist(.*?)letter=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{sucms:customvideolist(.*?)state=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{sucms:customvideolist(.*?)maxpage=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$labelRule = buildregx("{sucms:customvideolist(.*?)jq=(.*?) ","is");
	preg_match_all($labelRule,$str,$pzar);
...
{
	$str=str_replace("{sucms:currenttypeid}",$currentTypeId,$str);
	return $str;
}
...
{
	global $dsql;
	$rowc=$dsql->GetOne("select vod as dd from sucms_topic where id=".$topicId);
	$topicvod = $rowc['dd'];
...
{
	global $dsql;
	$cachefile = sucms_DATA.'/cron.cache.php';
	$row = $dsql->GetOne("SELECT nextrun,filename FROM sucms_crons WHERE available>'0' AND nextrun>'0' ORDER BY nextrun");
	...
	$cachedata = "<?php\r\n//ssucms2.0 cache file\r\n//Created on ".MyDate('Y-m-d H:i:s',time())."\r\n\r\nif(!defined('sucms_INC')) exit('Access Denied');\r\n\r\n".$contents."\r\n\r\n?>";
...
function RWCache($name,$value='') {
    static $_cache = array();
    $filename   =   sucms_DATA.'/cache/'.$name.'.php';
    if('' !== $value) {
        if(is_null($value)) {
            // 删除缓存
...
{
	@session_start();
	$usergroup=$_SESSION["sucms_user_group"];
...
{
	global $dsql;
	$dsql->SetQuery("select * from sucms_member_group");
...
{
	global $cfg_banIPS;
	$iplisttxt=sucms_DATA."/admin/iplist.txt";
