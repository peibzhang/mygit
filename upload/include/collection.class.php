...
class Collect
{
	...
	*/
		...
		if($html){
			...
			if((!$commconf['getherday'])||($commconf['getherday']&&$this->isLeftDates($pdate,$commconf['getherday']))){
			  ...
			  if(trim($itemconf["getpart"])==1 && trim($parta) !='' && trim($partb) != ''){
			      ...
			      $sql = "update `sucms_co_url` set succ='1' where uid=".$row['uid'];
				...
				$sql = "update `sucms_co_url` set err=err+1 where uid=".$row['uid'];
				$dsql->ExecuteNoneQuery($sql);
	...
	public function _insert_database($v_data)
	{
		...
		if(insert_record('sucms_data',$v_data))
		{
			$v_id = $dsql->GetLastID();
			...
			insert_record('sucms_content',$desdata);
			insert_record('sucms_playdata',$playdata);
...
//		}
		...
		$v_sql="select d.v_id,d.v_pic,d.v_isunion,p.body as v_playdata ,p.body1 as v_downdata from sucms_data d left join sucms_playdata p on p.v_id=d.v_id where $v_where order by d.v_id desc";
		...
		if(is_array($rs))
		{
			...
			{
				$v_sql1="select d.v_id,p.body as v_playdata,p.body1 as v_downdata,d.v_pic from sucms_data d left join sucms_playdata p on p.v_id=d.v_id where $v_where and d.tid='".$v_data['tid']."' order by d.v_id desc";
	...
	public function _into_tempdatabase($v_data,$i=0)
	{
		...
		$db = !empty($v_data['v_from'])  ? 'sucms_co_data':'sucms_temp';
		if($db == 'sucms_temp')
		{
			unset($v_data['tname']);
			$v_data['tid'] = '000000';
...
//		}
	...
	{
		update_record('sucms_data',"where v_id=".$rs['v_id'],array('v_ismake'=>'0','v_state'=>$v_data['v_state'],'v_note'=>$v_data['v_note'],'v_addtime'=>time()));
		update_record('sucms_playdata',"where v_id=".$rs['v_id'],array('body'=>$v_data['v_playdata'],'body1'=>$v_data['v_downdata']));
	...
	function update_movie_info($rs,$v_data)
	{
		...
		unset($v_data['v_playdata']);
		unset($v_data['v_downdata']);
		update_record('sucms_data',"where v_id=".$rs['v_id'],$v_data);
		update_record('sucms_playdata',"where v_id=".$rs['v_id'],array('body'=>$v_playdata,'body1'=>$v_downdata));
		update_record('sucms_content',"where v_id=".$rs['v_id'],array('body'=>$v_des));
	...
	function filterWord($string,$rCol)
	{
		...
		if($string=='')
		return $string;
		$sql = "SELECT rColumn,uesMode,sFind,sReplace,sStart,sEnd FROM sucms_co_filters WHERE Flag=1 and cotype=0";
		$dsql->SetQuery($sql);
