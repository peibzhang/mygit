...
class MainClass_Template
{
	...
	function parseTopAndFoot($content)
	{
		...
		$content=str_replace("{sucms:top}",loadFile($this->templateDir."head.html"),$content);
		$content=str_replace("{sucms:foot}",loadFile($this->templateDir."foot.html"),$content);
		if(strpos($content, "{sucms:load "))
		{
			preg_match_all("/{sucms:load (.*?)}/is", $content, $matches);
		...
		if($cfg_gbookstart){
			$content=str_replace("{sucms:gbook}","",$content);
		}else{
			$content=str_replace("{sucms:gbook}","<a href=\"/{sucms:sitepath}gbook.php\" target=\"_blank\">留言求片</a>",$content);
		}
	...
	function parseSelf($content){
		if (strpos($content,'{self:')=== false){
			...
			$sql="select tagname,tagcontent from sucms_mytag";
			if($cfg_issqlcache){
	...
	function parseStrip($content){
		if(strpos($content, "{/sucms:strip}")===false)
		{
			return $content;
		}
		else{
			...
			preg_match_all("@{sucms:strip}(.*?){/sucms:strip}@is", $content, $matches);
			foreach ($matches[1] as $k=>$match){
	...
	function parseGlobal($content){
		if (strpos($content,'{sucms:letterlist}')>0) $content=str_replace("{sucms:letterlist}",getletterlist(),$content);
		if (strpos($content,'{sucms:indexlink}')>0) $content=str_replace("{sucms:indexlink}",getIndexLink(),$content);
		if (strpos($content,'{sucms:newslink}')>0) $content=str_replace("{sucms:newslink}",getnewsxLink(),$content);		
		if (strpos($content,'{sucms:topiclink}')>0) $content=str_replace("{sucms:topiclink}",getTopicIndexLink(1),$content);
		$content=str_replace("{sucms:siteurl}",str_replace("http://","",$GLOBALS['cfg_basehost']),$content);
		$content=str_replace("{sucms:sitepath}",$GLOBALS['cfg_cmspath'],$content);
		$content=str_replace("{sucms:adfolder}",$GLOBALS['cfg_ads_dir'],$content);
		$content=str_replace("{sucms:sitename}",$GLOBALS['cfg_webname'],$content);
		$content=str_replace("{sucms:copyright}",decodeHtml($GLOBALS['cfg_powerby']),$content);
		$content=str_replace("{sucms:des}",decodeHtml($GLOBALS['cfg_description']),$content);
		$content=str_replace("{sucms:sitevisitjs}",stripslashes($GLOBALS['cfg_sitevisitejs']),$content);
		$content=str_replace("{sucms:sitenotice}",decodeHtml($GLOBALS['cfg_site_notice']),$content);
		if(strpos($content,"{sucms:hotkeywords")!==false)
		{
			$labelHaveLen = buildregx("{sucms:hotkeywords\s+len=(\d+)?\s*}","is");
			...
			{
				$content=str_replace("{sucms:hotkeywords}",lib_hotwords(),$content);	
			}
		}
		$content=str_replace("{sucms:keywords}",site_keywords(),$content);
		//$content=str_replace("{sucms:beian}",$GLOBALS['cfg_beian'],$content);
		if (strpos($content,'{sucms:allcount}')>0) $content=str_replace("{sucms:allcount}",getDataCount("all"),$content);
		if (strpos($content,'{sucms:daycount}')>0) $content=str_replace("{sucms:daycount}",getDataCount("day"),$content);
		...
		if($GLOBALS['cfg_runmode']=='0') $content=str_replace("{sucms:runinfo}","",$content);
	...
	function parseCascadeList($content,$str="")
	{
		...
		$labelRule="{sucms:".$str."caslist(.*?)}(.*?){/sucms:".$str."caslist}";
		...
		$arlen=count($mar[1]);
			for($m=0;$m<$arlen;$m++){
				...
				$attrDictionary=$this->parsucmsttr($attrStr);
				...
				switch(trim($str))
				{
					...
					break;
					case "lang":
						$publishyuyantxt=sucms_DATA."/admin/publishyuyan.txt";
						$publishyuyan = array();
					...
					break;
					case "ver":
						$vertxt=sucms_DATA."/admin/verlist.txt";
						$ver = array();
					...
					break;
					case "jq":
						$sql="select tname from sucms_jqtype where ishidden=0";
						$this->dsql->SetQuery($sql);
		...
		if(strpos($content,"{sucms:sectypecaslist")){
	...
	function parsesearchItemList($content,$str="")
	{
		...
		$labelRule="{sucms:".$str."itemlist(.*?)}(.*?){/sucms:".$str."itemlist}";
		$attrDictionary=array();
		$labelRule = buildregx($labelRule,"is");
		preg_match_all($labelRule,$content,$mar);
		$arlen=count($mar[1]);
		...
		if(strpos($content,"{sucms:sectypeitemlist")){
			return $this->parseCascadeList($content,"sectype");
		}else{
			return $content;
		}
	...
	{
		$labelRule="{sucms:".$str."menulist(.*?)}(.*?){/sucms:".$str."menulist}";
		$attrDictionary=array();
		$labelRule = buildregx($labelRule,"is");
		preg_match_all($labelRule,$content,$mar);
		$arlen=count($mar[1]);
		...
		if(strpos($content,"{sucms:smallmenulist")){
	...
	function parsucmsreaList($content){
		if (strpos($content,'{sucms:arealist')=== false){
			return $content;
		}else{
			$attrDictionary=array();
			$labelRule = buildregx("{sucms:arealist(.*?)}(.*?){/sucms:arealist}","is");
			$labelRuleVideolist=buildregx("{sucms:videolist(.*?)}(.*?){/sucms:videolist}","is");
				...
				for($j=0;$j<$letterlen;$j++){
					if(!empty($letterHasArray[$j])){
						...
						if(strpos($singleStrAreaList,"[arealist:count]")>0){
							$row=$this->dsql->GetOne("select count(*) as dd from `sucms_data` where v_letter='".$letterHasArray[$j]."'");
				...
				foreach($tagHas as $j=>$tg)
				{
					if(!empty($tg))
					{
						...
						if(strpos($tmp,"[arealist:count]")>0){
							$row=$this->dsql->GetOne("SELECT COUNT(*) as dd FROM sucms_data WHERE v_name LIKE '%".$tagHas[$j]."%' OR m_actor like '%".$tagHas[$j]."%' OR  m_director like '%".$tagHas(j)."%'");
						}
	...
	function parseNewsAreaList($content){
		if (strpos($content,'{sucms:newsarealist')=== false){
			return $content;
		}else{
			$attrDictionary=array();
			$labelRule = buildregx("{sucms:newsarealist(.*?)}(.*?){/sucms:newsarealist}","is");
			$labelRuleVideolist=buildregx("{sucms:newslist(.*?)}(.*?){/sucms:newslist}","is");
			preg_match_all($labelRule,$content,$ar);
			$arlen=count($ar[1]);
			...
			if($areaType=="letter"){
				...
				for($j=0;$j<$letterlen;$j++){
					if(!empty($letterHasArray[$j])){
						...
						$singleStrAreaList=str_replace("[arealist:typename]",$letterHasArray[$j],$singleStrAreaList);
						if(strpos($singleStrAreaList,"[arealist:count]")>0){
							$row=$this->dsql->GetOne("select count(*) as dd from `sucms_news` where n_letter='".$letterHasArray[$j]."'");
							$singleStrAreaList=str_replace("[arealist:count]",$row['dd'],$singleStrAreaList);
	...
	function parseNewsList($content,$currentTypeId=-444,$vtag){
		if (strpos($content,'{sucms:newslist')=== false){
			return $content;
		}else{
		global $cfg_issqlcache;
		$attrDictionary=array();
		$labelRule = buildregx("{sucms:newslist(.*?)}(.*?){/sucms:newslist}","is");
		preg_match_all($labelRule,$content,$ar);
		$arlen=count($ar[1]);
		for($m=0;$m<$arlen;$m++){
			$attrStr = trim(preg_replace("/[ \r\n\t\f]{1,}/"," ",$ar[1][$m]));
			...
			$sql="select m.n_id,m.n_title,m.n_pic,m.tid,m.n_hit,m.n_author,m.n_color,m.n_addtime ,m.n_commend,m.n_note,m.n_letter,m.n_content,m.n_outline,m.n_from from sucms_news m ".$whereStr.$orderStr." LIMIT $vstart,$vnum";
			if($cfg_issqlcache){
	...
	function parseVideoList($content,$currentTypeId=-444,$topicId=10,$vtag){
		if (strpos($content,'{sucms:videolist')=== false){
			return $content;
		}else{
			global $cfg_issqlcache,$cfg_runmode,$id,$vid;
			$attrDictionary=array();
			$labelRule = buildregx("{sucms:videolist(.*?)}(.*?){/sucms:videolist}","is");
			preg_match_all($labelRule,$content,$ar);
			$arlen=count($ar[1]);
			for($m=0;$m<$arlen;$m++){
				$attrStr = trim(preg_replace("/[ \r\n\t\f]{1,}/"," ",$ar[1][$m]));
				...
				if (strpos($loopstrVideoList,"[videolist:des")>0){
					...
					$left_des=" left join `sucms_content` c on c.v_id=m.v_id ";
				}else{
					$field_des=0;
					$left_des="";
				}
				if (strpos($loopstrVideoList,"[videolist:from]")>0){
					...
					$left_playdata=" left join `sucms_playdata` p on p.v_id=m.v_id ";
				}else{
					$field_playdata=0;
					$left_playdata="";
				}
				...
				if(!empty($vrel))
				{
					$zid=$GLOBALS['zid'];	
				if($id !=0 or $id !="")
					{$sql="select v_name,v_actor,v_director from sucms_data where v_id=$id limit 1";}
				if($vid !=0 or $vid !="")
					{$sql="select v_name,v_actor,v_director from sucms_data where v_id=$vid limit 1";}
				if($zid !=0 or $zid !="")
					{$sql="select v_name,v_actor,v_director from sucms_data where v_id=$zid limit 1";}
					$zrel=array();
					$this->dsql->SetQuery($sql);
					$this->dsql->Execute('al');
				...
				if(!empty($vtopic))
				{
					$sql="select vod from sucms_topic where id=$vtopic limit 1";
					$this->dsql->SetQuery($sql);
					$this->dsql->Execute('al');
				...
				if(trim($whereStr)=="where") $whereStr="";
				$sql="select m.*,".$field_des.",".$field_playdata." from sucms_data m ".$left_des.$left_playdata.$whereStr.$orderStr." LIMIT $vstart,$vnum";
				//echo $sql.'<br>';
				...
				foreach($rows as $row)
				{
					...
					foreach($matchfieldarr as $f=>$matchfieldstr){
						...
						switch (trim($fieldName)) {
							...
							case "favLink":
								@session_start();
								if(isset($_SESSION['sucms_user_auth'])||$GLOBALS['cfg_user']==1)
								{														
									$uid=$_SESSION['sucms_user_id'];
	...
	function parsePageList($content,$typeId,$currentPage,$TotalPage,$TotalResult,$pageListType,$currentTypeId=-444)
	{
		...
		$attrDictionary=array();
		$labelRule = buildregx("{sucms:".$pageListType."list(.*?)}(.*?){/sucms:".$pageListType."list}","is");
		...
		for($m=0;$m<$arlen;$m++){
			...
			switch ($pageListType) {
				...
				case "topicpage":
						...
						//echo '====='.$typeId.'===='; //专题id
						$sql="select vod from sucms_topic where id='$typeId'";
						$rows=array();
						$this->dsql->SetQuery($sql);
						$this->dsql->Execute('al');
						while($rowr=$this->dsql->GetObject('al'))
			...
			$sql="select m.*,".$field_des.",".$field_playdata." from sucms_data m ".$left_des.$left_playdata.$whereStr." ".$orderStr." limit $limitstart,$vsize";
		...
		if(strpos($content,"{/".$pageListType."list:pagenumber}")>0)
		{
			...
			foreach ($matchesPagelist[1] as $k=>$matchPagelist){
				$attr=$this->parsucmsttr($matchPagelist);
				$len=$attr['len'];
	...
	function parseNewsPageList($content,$typeId,$currentPage,$totalPages,$pageListType,$currentTypeId=-444)
	{
		...
		for($m=0;$m<$arlen;$m++){
			...
			$sql="select * from sucms_news ".$whereStr." ".$orderStr." limit $limitstart,$row";
			$cquery = "Select count(*) as dd From `sucms_news` ".$whereStr;
	...
	{
		global $lang;
		if (strpos($content,'{sucms:topiclist')=== false){
			return $content;
		}else{
			global $cfg_issqlcache;
			$attrDictionary=array();
			$labelRule = buildregx("{sucms:topiclist(.*?)}(.*?){/sucms:topiclist}","is");
			preg_match_all($labelRule,$content,$ar);
			$arlen=count($ar[1]);
			for($m=0;$m<$arlen;$m++){
				$attrStr = trim(preg_replace("/[ \r\n\t\f]{1,}/"," ",$ar[1][$m]));
				...
				$cquery = "Select count(*) as dd From `sucms_topic`";
				$row = $this->dsql->GetOne($cquery);
				if(is_array($row))
				{
					$TotalResult = $row['dd'];
				...
				$sql="select name,id,pic,enname,des from sucms_topic".$whereTopic." order by sort desc limit $start,$num";
				if($TotalResult == 0){
	...
	{
		global $lang;
		if (strpos($content,'{sucms:topicindexlist')=== false){
			return $content;
		}else{
			global $cfg_issqlcache;
			$attrDictionary=array();
			$labelRule = buildregx("{sucms:topicindexlist(.*?)}(.*?){/sucms:topicindexlist}","is");
			preg_match_all($labelRule,$content,$ar);
			$arlen=count($ar[1]);
			for($m=0;$m<$arlen;$m++){
				$attrStr = trim(preg_replace("/[ \r\n\t\f]{1,}/"," ",$ar[1][$m]));
				...
				$TotalPage = ceil($TotalResult/$vsize);
				$limitstart = ($currentPage-1) * $vsize;
				$sql="select name,id,pic,enname,des from sucms_topic order by sort desc limit $limitstart,$vsize";
				if($TotalResult == 0){
				$loopstrTotal=$lang['topicpageInfo']['0'];
				}else{
					if($cfg_issqlcache){
	...
	function paresPreNextVideo($content,$dataId,$typeFlag,$vtype)
	{
		...
		if (strpos($content,$preNextLabel)=== false){
			...
			$rown = $this->dsql->GetOne("select v_id as nextid ,v_name as nextname,v_addtime,v_enname from sucms_data where tid='$vtype' and v_id<".$dataId." order by v_id desc");
			if(is_array($rown)){
				$nextid=$rown["nextid"];
			...
			$rowl = $this->dsql->GetOne("select v_id as lastid ,v_name as lastname,v_addtime,v_enname from sucms_data where tid='$vtype' and v_id>".$dataId." order by v_id asc");
			if(is_array($rowl)){
				$lastid=$rowl["lastid"];
	...
	function parseLinkList($content){
		global $cfg_issqlcache;
		if (strpos($content,'{sucms:linklist')=== false){
		return $content;
		}else{
		$attrDictionary=array();
		$labelRule = buildregx("{sucms:linklist(.*?)}(.*?){/sucms:linklist}","is");
		preg_match_all($labelRule,$content,$ar);
		$arlen=count($ar[1]);
		for($m=0;$m<$arlen;$m++){
		$attrStr = trim(preg_replace("/[ \r\n\t\f]{1,}/"," ",$ar[1][$m]));
		...
		$sql="select url,webname,msg,logo from sucms_flink where ".$whereStr." order by sortrank";
		...
		for($m=0;$m<$arlen;$m++){
			...
			if (strpos($strThen,$labelRule2)===false){
				if (strpos($strThen,$labelRule3)>=0){
					$elsucmsrray=explode($labelRule3,$strThen);
					$strThen1=$elsucmsrray[0];
					$strElse1=$elsucmsrray[1];
	...
	function parseSlide($content){
		if (strpos($content,'{sucms:slide')=== false){
		return $content;
		}else{
			$slideStr2=getSubStrByFromAndEnd_en($content,"{sucms:slide","}","");
			$slidewarr=$this->parsucmsttr($slideStr2);
			$slidew=$slidewarr['width'];
			if(empty($slidew)) $slidew='400';
			$slideharr=$this->parsucmsttr($slideStr2);
			$slideh=$slideharr['height'];
			if(empty($slideh)) $slideh='280';
			$content=str_replace("{sucms:slide".$slideStr2."}","<script>loadSlide('".$slidew."','".$slideh."',sitePath)</script>",$content);
			return $content;
		}
	}
	...
	function parseCommentList($content,$vId,$currentPage,$totalPages,$pageListType,$type=0){
		...
		for($m=0;$m<$arlen;$m++){
			...
			$sql="select * from `sucms_comment` where v_id='$vId' and ischeck='1' ".$orderStr." limit $limitstart,$row";
			$cquery = "Select count(*) as dd From `sucms_comment` where v_id='$vId' and ischeck='1'";
			$row = $this->dsql->GetOne($cquery);
			if(is_array($row))
			{
				$TotalResult = $row['dd'];
	...
	function parseHistory($content){
		$content=str_replace("{sucms:showhistory}","<a href=\"javascript:void(0)\" onclick=\"\$MH.showHistory(1);\">我的观看历史</a>",$content);
		if (strpos($content,'{sucms:maxhistory')=== false){
			return $content;
		}else{
			$y=getSubStrByFromAndEnd_en($content, "{sucms:maxhistory","}","");
			$q=$this->parsucmsttr($y);
			$w=$q['width'];
			$h=$q['height'];
			$i=$q['num'];
			...
			if(empty($h)) $h=170;
			if(empty($i)) $i=10;
			$content=str_replace("{sucms:maxhistory".$y."}","<script type=\"text/javascript\" src=\"/".$GLOBALS['cfg_cmspath']."js/history.js\"></script><script type=\"text/javascript\">\$MH.limit=".$i.";\$MH.WriteHistoryBox(".$w.",".$h.",'".$c."');\$MH.recordHistory({name:'{playpage:name}',link:'{playpage:url}',pic:'{playpage:pic}'})</script>",$content);
			return $content;
		}
		return $content;
	}
	...
	function paresPreNextNews($content,$dataId,$typeFlag,$vtype)
	{
		...
		if (strpos($content,$preNextLabel)=== false){
			...
			$rown = $this->dsql->GetOne("select n_id as nextid ,n_title as nextname from sucms_news where tid='$vtype' and n_id<".$dataId." order by n_id desc");
			if(is_array($rown)){
				$nextid=$rown["nextid"];
				$nextname=$rown["nextname"];
			}else{
			...
			$rowl = $this->dsql->GetOne("select n_id as lastid ,n_title as lastname from sucms_news where tid='$vtype' and n_id>".$dataId." order by n_id asc");
			if(is_array($rowl)){
				$lastid=$rowl["lastid"];
				$lastname=$rowl["lastname"];
			}else{
	...
	function paresVideoInNews($content)
	{
		...
		if (strpos($content,'{news:video')=== false){
			...
			$vidArr=explode(',',$vid);
			$row=$dsql->GetOne("select body as v_playdata from sucms_playdata where v_id=".$vidArr[0]);
