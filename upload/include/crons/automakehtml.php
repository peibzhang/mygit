...
@set_time_limit(0);
if(!class_exists("MainClass_Template")) require_once(sucms_INC.'/main.class.php');

//清理缓存
autocache_clear(sucms_ROOT.'/data/cache');
//生成首页
makeIndex();
...
function automakeDay()
{
	...
	$pagesize=100;
	if(!$pCount){
	$rowc=$dsql->GetOne("SELECT count(*) as dd FROM `sucms_data` WHERE `v_wrong`=0 ".$wheresql);
	$totalnum = $rowc['dd'];
	...
	$sql="select v_id from sucms_data where v_wrong=0 $wheresql";
	$dsql->SetQuery($sql);
	...
	$ids="";
	$sqlt="SELECT tid from sucms_data where v_wrong=0 ".$wheresql." GROUP BY tid";
	$dsql->SetQuery($sqlt);
...
function automakeTopicIndex()
{
	...
	for($i=1;$i<=$pCount;$i++)
	{
		...
		$content=$mainClassObj->parseIf($content);
		if($i==1)$topicindexname=sucms_ROOT."/".$GLOBALS['cfg_album_name']."/index".$GLOBALS['cfg_filesuffix2'];
		else $topicindexname=sucms_ROOT."/".$GLOBALS['cfg_album_name']."/index".$i.$GLOBALS['cfg_filesuffix2'];
...
{
	global $dsql,$cfg_iscache,$mainClassObj;
	$sql="select id,name,template,enname from sucms_topic where id =".$topicId;
	$row = $dsql->GetOne($sql);
	if(!is_array($row)) return FALSE;
	$rowc=$dsql->GetOne("select count(*) as dd from sucms_data where v_topic=".$topicId);
...
function automakeCustomInfo($templatename)
{
	...
	if(strpos($content, "{/sucms:customvideolist}")>0){
		...
		if (empty($pSize)) $pSize=12;
		$sql="select count(*) as dd from sucms_data where v_recycled=0";
		$row = $dsql->GetOne($sql);
		if(is_array($row))
		{
			$TotalResult = $row['dd'];
