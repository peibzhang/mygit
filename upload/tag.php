...
function echoTagPage()
{
	...
	$sql="select vids from sucms_tags where tag='$tag'";
	$row = $dsql->GetOne($sql);
	if(is_array($row))
	{
		$vids=$row['vids'];
	...
	$tempStr = $content;
	$tempStr = str_replace("{sucms:tag}",$tag,$tempStr);
	$tempStr = str_replace("{sucms:tagnum}",$TotalResult,$tempStr);
	$content=$tempStr;
...
{
	if(GetCookie("ssucms2_tag")=="ok")
	{
		ShowMsg('搜索限制为'.$searchtime.'秒一次','-1','0',$cfg_search_time);
		exit;
	}else{
		PutCookie("ssucms2_tag","ok",$searchtime);
	}
