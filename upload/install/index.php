...
define('sucms_INC',dirname(__FILE__).'/../include');
define('sucms_DATA',dirname(__FILE__).'/../data');
define('sucms_ROOT',preg_replace("|[\\\/]install|",'',dirname(__FILE__)));
header("Content-Type: text/html; charset={$s_lang}");

require_once(sucms_ROOT.'/install/install.inc.php');
...
{
	exit(" SuCms已成功安装，如果需要重新安装，请删除根目录 install/install_lock.txt！");
}
...
{
	@include sucms_DATA.'/config.ucenter.php';
...
{
  @include sucms_DATA.'/config.ucenter.php';
  $configfile = sucms_DATA.'/config.ucenter.php';
  ...
  @chmod(sucms_ROOT.'/data',0777);
  $fp = fopen(sucms_ROOT."/data/common.inc.php","w") or die("<script>alert('写入配置失败，请检查../data目录是否可写入！');history.go(-1);</script>");
  ...
  fclose($fp);
	...
	$fp = fopen(sucms_ROOT.'/data/config.cache.inc.php','w');
  fwrite($fp,$configStr2);
  fclose($fp);

  $fp = fopen(sucms_ROOT.'/data/config.cache.bak.php','w');
  fwrite($fp,$configStr2);
  fclose($fp);
  ...
  $query = '';
  $fp = fopen(dirname(__FILE__).'/sucms.sql','r');
	while(!feof($fp))
	{
		 ...
		 {
			   $query .= $line."\n";
			   $query = str_replace('sucms_',$dbprefix,$query);
			   if($mysqlVersion < 4.1)
			   {
	...
	//默认数据库
	$query = '';
	$fp = fopen(dirname(__FILE__).'/sucmsdata.sql','r');
	while(!feof($fp))
	{
		 $line = rtrim(fgets($fp,1024));
		 if(m_ereg(";$",$line))
	...
	$flinkquery = "INSERT INTO `{$dbprefix}flink` (`id`, `sortrank`, `url`, `webname`, `msg`, `email`, `logo`, `dtime`, `ischeck`) VALUES (NULL, '0', 'http://www.sucms.org', '速成影视管理系统', '', '', '', '1432312055', '1');";
