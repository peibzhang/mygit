...
function echosearchPage()
{
	...
	$sql="select count(*) as dd from sucms_data ".$whereStr;
	$row = $dsql->GetOne($sql);
	if(is_array($row))
	{
		$TotalResult = $row['dd'];
	...
	$content = str_replace("{sucms:searchword}",$searchword,$content);
	$content = str_replace("{sucms:searchnum}",$TotalResult,$content);
	...
	echo str_replace("{sucms:runinfo}",getRunTime($t1),$searchPageStr) ;
}
...
{
	if(GetCookie("ssucms2_search")=="ok")
	{
		ShowMsg('搜索限制为'.$searchtime.'秒一次','index.php','0',$cfg_search_time);
		//PutCookie("ssucms2_search","ok",$searchtime);
		exit;
	}else{
		PutCookie("ssucms2_search","ok",$searchtime);
	}
