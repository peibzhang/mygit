...
elseif($action=="bindsubmit")
{
	if(is_numeric($tid))
	{
		if(!empty($v_oldtype))
		{
			$row = $dsql->GetOne("select tid,unionid from sucms_type where tid='$v_oldtype'");
			if(is_array($row)){
				$unionid=delBindId($curid,$row["unionid"]);
				$dsql->ExecuteNoneQuery("update sucms_type set unionid='$unionid' where tid=".$row['tid']);
			}
		}
		$row = $dsql->GetOne("select tid,unionid from sucms_type where tid='$tid'");
		if(is_array($row)){
		...
		$row = $dsql->GetOne("select tid,unionid from sucms_type where concat(',',unionid,',') like '%,".$curid.",%'");
		if(is_array($row)){
			$unionid=delBindId($curid,$row["unionid"]);
			$dsql->ExecuteNoneQuery("update sucms_type set unionid='$unionid' where tid=".$row['tid']);
		}
...
{
	global $dsql;
	$row = $dsql->GetOne("select count(*) as dd from sucms_type where unionid<>''");
	if(!is_array($row)) return '';
	$sqlStr="select tid,unionid from sucms_type where unionid<>''";
...
<div align=center>Copyright 2011-2012 All rights reserved. <a target="_blank" href="http://www.sucms.org">sucms.org</a></div>
</body>
</html> 
<?php
}
