...
require_once(sucms_INC."/image.func.php");
if($cfg_photo_support=='')
{
	echo "你的系统没安装GD库，不允许使用本功能！";
	exit();
}
$ImageWaterConfigFile = sucms_DATA."/mark/inc_photowatermark_config.php";
if(empty($dopost))
{
	$dopost = "";
}
if($dopost=="save")
{
	...
	if(is_uploaded_file($newimg))
	{
		...
		@move_uploaded_file($newimg,sucms_DATA."/mark/".$photo_markimg);
	}
...
include(sucms_ADMIN.'/templets/admin_config_mark.htm');
exit();
?>