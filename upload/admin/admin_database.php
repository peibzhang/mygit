...
if($action=="optimize")
{
	...
	include(sucms_ADMIN.'/templets/admin_database_optimize.htm');
	unset($showtablerow);
	exit();
}
elseif($action=="bak")
{
	$bkdir = sucms_DATA.'/'.$cfg_backup_dir;
	...
	$bkfile = !isset($bkfile)?($bkdir."/sucms_data_$nowtime.txt"):$bkfile;
	//第一页的操作
	if($nowtable=='')
	{
		$tmsg = '';
...
*/
		if($isstruct==1)
		{
			$bkstrufile = $bkdir."/sucms_tables_struct_$nowtime.txt";
	...
	else
	{
		...
		while($row2 = $dsql->GetArray())
		{
			...
			{
				$nowtime = date('Y_m_d_H_i_s',time());
				$bkfile = $bkdir."/sucms_data_".$nowtime."_".$m.".txt";
				$fp = fopen($bkfile,"a");
				fwrite($fp,$bakStr);
				fclose($fp);
...
elseif($action=="import")
{
	...
	$dh->close();
	include(sucms_ADMIN.'/templets/admin_database_import.htm');
	exit();
}
elseif($action=="redat")
{
	...
	$structfile = "sucms_tables_struct_".str_replace("sucms_data_","",$bakfiles[0]);
...
else
{
	...
	include(sucms_ADMIN.'/templets/admin_database.htm');
	exit();
}
...
function PutInfo($msg1,$msg2)
{
	...
	$msginfo = "<html>\n<head>
		...
		<title>sucms 提示信息</title>
		<base target='_self'/>\n</head>\n<body leftmargin='0' topmargin='0'>\n<center>
		<br/>
		<div style='width:400px;padding-top:4px;height:24;font-size:10pt;border-left:1px solid #cccccc;border-top:1px solid #cccccc;border-right:1px solid #cccccc;background-color:#DBEEBD;'>sucms 提示信息！</div>
