...
if($action=="add")
{
	...
	if(empty($torder)) 
		{
		$trow = $dsql->GetOne("select max(torder)+1 as dd from sucms_type");
		$torder = $trow['dd'];
		}
	if (!is_numeric($torder)) $torder=1;
	$in_query = "insert into `sucms_type`(upid,torder,tname,tenname,templist,templist_1,title,keyword,description,tptype) Values('$upid','$torder','$tname','$tenname','$templist','$templist_1','$title','$keyword','$description',0)";
...
elseif($action=="hide")
{
	...
	$dsql->ExecuteNoneQuery("update sucms_type set ishidden=1 where tid=".$id);
	if (!empty($typePath))
	{
		delFolder("../".$typePath);
	}
...
{
	$dsql->ExecuteNoneQuery("update sucms_type set ishidden=0 where tid=".$id);
	clearTypeCache();
	header("Location:admin_type.php");
	exit;
}
...
elseif($action=="move")
{
	...
	foreach($e_id as $id)
	{
		...
		if(strpos($subTypeList,",".$upid_to.",")<=0) $dsql->ExecuteNoneQuery("update sucms_type set upid=".$upid_to." where tid=".$id);
	}
	clearTypeCache();
...
elseif($action=="edit")
{
	...
	if (!is_numeric($torder)) $torder=1;
	$dsql->ExecuteNoneQuery("update sucms_type set tname='$tname',tenname='$tenname',templist='$templist',templist_1='$templist_1',keyword='$keyword',title='$title',description='$description',torder='$torder' where tid=".$id);
	clearTypeCache();
	}
	header("Location:admin_type.php");
	exit;
...
elseif($action=="movevideo")
{
	...
	$dsql->ExecuteNoneQuery("update sucms_data set tid='$rightSelect' where tid=".$leftSelect);
	if (!empty($typePath))
	{
		delFolder("../".$typePath);
	}
...
{
include(sucms_ADMIN.'/templets/admin_type.htm');
exit();
}
...
function clearTypeCache()
{
	...
	if($cfg_iscache)
	{
		$TypeCacheFile=sucms_DATA."/cache/".$cfg_cachemark."obj_get_type_list_0.inc";
...
function delVideoTypeById($id)
{
	...
	if (isExistSub($id,"type"))
	{
		...
		$dsql->ExecuteNoneQuery("delete from sucms_type where tid=".$id);
		clearTypeCache();
		if (!empty($typePath))
		{
			delFolder("../".$typePath);
...
function isExistSub($bigType,$operFlag)
{
	...
	{
		case "type":
			$row = $dsql->GetOne("select count(*) as dd from sucms_type where upid=".$bigType);
		break;
		case "video":
			$row = $dsql->GetOne("select count(*) as dd from sucms_data where tid=".$bigType);
		break;
	}
	if($row[dd]>0)
	{
