...
if($ac=='search')
{
	...
	$sql = "select * from sucms_member $wheresql";
	$dsql->SetQuery($sql);
...
elseif($ac=='clear')
{
	...
	$sql = "delete from sucms_member $wheresql";
...
{
	$row = $dsql->GetOne("select * from sucms_member where id='$id'");
}
elseif($ac=='editsave')
{
	$gid = intval($gid);
	...
	if($dsql->ExecuteNoneQuery("update sucms_member set gid=$gid,points=$upoints,state=$ustate where id=$id"))
	{
...
{	
	$sql = "delete from sucms_member where id='$id'";
...
include(sucms_ADMIN.'/templets/admin_members.htm');
exit();
