...
elseif($action=="daysview")
{
	...
	$sql = "SELECT v_id FROM sucms_data where v_addtime>".$limitday;
	$dsql->SetQuery($sql);
...
elseif($action=="newsdaysview")
{
	...
	$ntime = gmmktime(0, 0, 0, gmdate('m'), gmdate('d'), gmdate('Y'));
	$limitday = $ntime - ($days * 24 * 3600);
	$sql = "SELECT n_id FROM sucms_news where n_addtime>".$limitday;
	$dsql->SetQuery($sql);
...
{
include(sucms_ADMIN.'/templets/admin_makehtml.htm');
exit();
}
?>