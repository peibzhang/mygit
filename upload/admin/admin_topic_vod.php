...
if(empty($action))
{
	...
	$sql="select * from sucms_topic where id=$tid"; //获取之前的影片id'数据
	$dsql->SetQuery($sql);
	$dsql->Execute('al') ;
...
if($action=="topicadd")
{
	...
	$zinfo="$info1$vid";
	$usql = "update sucms_topic set vod='$zinfo' where id='$tid'";
...
elseif($action=="topicdel")
{
	...
	$sql = "update sucms_topic set vod='$del' where id='$tid'";
...
{	
	...
	$all="$info1$allstr";
	$usql = "update sucms_topic set vod='$all' where id='$tid'";
	if(!$dsql->ExecuteNoneQuery($usql))
	{echo "'alert('插入数据失败')";}
	header("Location: admin_topic_vod.php?tid=$tid"); 
	exit;	
...
{
	$dallsql = "update sucms_topic set vod='0' where id='$tid'";
...
include(sucms_ADMIN.'/templets/admin_topic_vod.htm');
exit();
}
elseif($action=="save")
{
	...
	{
		case "add":
			$insertSql = "insert into sucms_data(tid,v_name,v_letter,v_state,v_topic,v_hit,v_money,v_rank,v_actor,v_color,v_publishyear,v_publisharea,v_pic,v_addtime,v_note,v_tags,v_lang,v_director,v_enname,v_commend,v_extratype) values ('$tid','$v_name','$v_letter','$v_state','$v_topic','$v_hit','$v_money','$v_rank','$v_actor','$v_color','$v_publishyear','$v_publisharea','$v_pic','$v_addtime','$v_note','$v_tags','$v_lang','$v_director','$v_enname','$v_commend','$v_extratype')";
