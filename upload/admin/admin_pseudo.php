...
$RepWordFile=sucms_DATA."/admin/repword.txt";
...
{
	$pagesize = 30;
	$sql = " select d.v_id,d.v_name,c.body as v_content from `sucms_data` d left join `sucms_content` c on c.v_id=d.v_id";
	$dsql->SetQuery($sql);
	...
	$sql = "select d.v_id,d.v_name,c.body as v_content from `sucms_data` d left join `sucms_content` c on c.v_id=d.v_id ".$whereStr." order by v_id ASC limit $page $pagesize";
	$dsql->SetQuery($sql);
...
{
	include(sucms_ADMIN.'/templets/admin_pseudo.htm');
	exit();
}