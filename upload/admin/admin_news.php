...
if($action=="add")
{
include(sucms_ADMIN.'/templets/admin_news_add.htm');
exit();
}
elseif($action=="edit")
{
	...
	//读取文章信息
	$query = "select * from sucms_news  where n_id='$id' ";
	...
	include(sucms_ADMIN.'/templets/admin_news_edit.htm');
	exit();
}
elseif($action=="save")
{
	...
	switch (trim($acttype)) 
	{
		case "add":
			...
			{
				$gerr = $dsql->GetError();
				ShowMsg("把数据保存到数据库主表 `sucms_news` 时出错。".str_replace('"','',$gerr),"javascript:;");
				exit();
			}
		break;
		case "edit":
			...
			$updateSql = "update sucms_news set ".$updateSql." where n_id=".$n_id;
//			echo $updateSql;die();
...
elseif($action=="del")
{
	$back=$Pirurl;
	$id = isset($id) && is_numeric($id) ? $id : 0;
	$vtypeAndPic=$dsql->GetOne("select tid,n_pic from sucms_news where n_id=".$id);
	...
	$dsql->ExecuteNoneQuery("delete from sucms_news where n_id=".$id);
	clearTypeCache();
	ShowMsg("文章删除成功",$back);
	exit();
}
elseif($action=="restoreall")
{
	...
	$ids = implode(',',$e_id);
	$sqlStr="update sucms_news set n_recycled=0 where n_id in(".$ids.")";
...
elseif($action=="delall")
{
	...
	$ids = implode(',',$e_id);
	$dsql->ExecuteNoneQuery("delete from sucms_news where n_id in(".$ids.")");
	clearTypeCache();
	ShowMsg("文章删除成功",$back);
	exit();
}
elseif($action=="psettype")
{
	...
	$ids = implode(',',$e_id);
	$dsql->ExecuteNoneQuery("update sucms_news set tid=".$movetype." where n_id in(".$ids.")");
...
elseif($action=="deltypedata")
{
	...
	$dsql->ExecuteNoneQuery("delete from sucms_news where tid=".$movetype);
...
elseif($action=="hide")
{
	...
	$dsql->ExecuteNoneQuery("update sucms_news set n_recycled=1 where n_id=".$id);
	ShowMsg("隐藏文章成功",$back);
	exit();
...
elseif($action=="restore")
{
	...
	$dsql->ExecuteNoneQuery("update sucms_news set n_recycled=0 where n_id=".$id);
	ShowMsg("还原文章成功",$back);
	exit();	
}
else
{
	include(sucms_ADMIN.'/templets/admin_news.htm');
	exit();
}
