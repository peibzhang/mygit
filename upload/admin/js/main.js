...
function openUpdateWin(width, height,str){
	...
	var str="<div style='width:400px;'><div class='divboxtitle'><span onclick=\"closeWin();set(document.getElementById('update'),'<font color=red >版本暂未升级...</font>');\" ><img src='../pic/btn_close.gif'/></span>sucms在线升级</div><div  class='divboxbody'>"+str+"<input type='button' value='进入升级页面' id='openwin' class='rb1'/>&nbsp;&nbsp;&nbsp;&nbsp;<input id='closewin' type='button' value='取   消' name='button' class='rb1'  /></div><div class='divboxbottom'>Power By sucms</div></div>"
...
function openCollectWin(width, height,str,url){
	...
	var bgDiv=document.getElementById("bg")
	var iWidth = document.documentElement.scrollWidth
	var str="<div style='width:400px;'><div class='divboxtitle'><span onclick=\"closeWin();\" ><img src='../pic/btn_close.gif'/></span>sucms温馨提示</div><div  class='divboxbody'>"+str+"<br><input type='button' value='继续采集' id='openwin' class='btn'/>&nbsp;&nbsp;<input id='closewin' type='button' value='取   消' name='button' class='btn'  />&nbsp;&nbsp;<input id='clearColHis' type='button' value='取消记录' name='button' class='btn'  /></div><div class='divboxbottom'>Power By sucms</div></div>"
	msgDiv.style.cssText += "FONT-SIZE: 12px;top:100px;left:"+(iWidth-width)/2+"px;text-align:center;";
	set(msgDiv,str)
	document.getElementById("closewin").onclick = function(){
		closeWin()
...
/*	if(str.indexOf('新浪高清')>-1) return "hd_iask"
	...
	if(str.indexOf('速成影音')>-1) return "ssucms"
*/	return ""
}
