...
$playerKindsfile = sucms_DATA."/admin/playerKinds.xml";
if($action=="edit")
{
	...
	$player=preg_replace("/sucms_Player_File=(.*?)\";/is","sucms_Player_File=\"$playerset\";",$player);
...
elseif($action=="save")
{
	...
	$sql = "select * from `sucms_playdata` ";
...
{
	include(sucms_ADMIN.'/templets/admin_player.htm');
	exit();
}
...
else
{
	...
	$playerset=getrulevalue($player,"sucms_Player_File=\"","\";");
