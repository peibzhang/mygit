...
if($action=="import")
{
	...
	$ids = implode(',',$e_id);
	$tid=$type;
	$sql="SELECT v_id,tid,v_name,v_letter,v_state,v_pic,v_actor,v_des,v_playdata,v_publishyear,v_publisharea,v_note,v_enname,v_lang,v_note,v_director FROM sucms_temp WHERE v_id IN (".$ids.")";
	$dsql->SetQuery($sql);
	...
	while($row=$dsql->GetAssoc('import_list'))
	{
		...
		$dsql->ExecuteNoneQuery("delete from `sucms_temp` where v_id=".$row['v_id']);
		@ob_flush();
	    @flush();
	}
...
elseif($action=="del")
{
	$dsql->ExecuteNoneQuery("delete from `sucms_temp` where v_id='$id'");
...
elseif($action=="delall")
{
	if($do=="all")
	{
		$dsql->ExecuteNoneQuery("delete from `sucms_temp`");
	}else{
		if(empty($e_id))
		{
		...
		$ids = implode(',',$e_id);
		$dsql->ExecuteNoneQuery("delete from `sucms_temp` where v_id in ($ids)");
	}
	header("Location:$Pirurl");
	exit;
}
elseif($action=="edit")
{
	...
	//读取影片信息
	$query = "select * from sucms_temp where v_id='$id' ";
	...
	include(sucms_ADMIN.'/templets/admin_tempvideo_edit.htm');
	exit();
}
elseif($action=="save")
{
	...
	$updateSql = "update sucms_temp set ".$updateSql." where v_id=".$v_id;
...
{
	include(sucms_ADMIN.'/templets/admin_tempvideo.htm');
	exit();
}
