...
function member()
{
	ajax.get( 
		...
		function (obj){
			var result=obj.responseText;
			set(document.getElementById('sucms_member'),result);
		}
	);
}
...
function showpf()
	{document.getElementById('sucmsvpf1').style.display="none";document.getElementById('sucmsvpf2').style.display="inline";}
function markscore1(vd,d,t,s,l,ac){
	...
	document.write('<span id="sucmsvpf1" onmouseover=showpf()></span>');
	document.write('<span id="sucmsvpf2">');
	for(var i=1;i<=l;i++){
...
function markNews2(vid,style,len){
	...
	ajax.get(
		...
		function (obj){
			...
			if(style==1){
			//星星评分
			document.getElementById('sucmsvpf2').style.display="none";
			id='STAR'+(new Date()).getTime();
			for(var ii=1;ii<=len;ii++){
			if(ii>sc){p=1;}else{p=0;}
			document.getElementById('sucmsvpf1').innerHTML+='<img iid='+ii+' src="/pic/star'+p+'.gif" style="cursor:pointer">';
			}
