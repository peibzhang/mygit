...
require_once(sucms_INC."/filter.inc.php");
...
if($action=='add')
{
	...
	{
		$row = $dsql->GetOne("SELECT dtime FROM `sucms_guestbook` WHERE `ip` = '$ip' ORDER BY `id` DESC ");
	...
	if($reid != 0)
	{
		$row = $dsql->GetOne("Select msg From `sucms_guestbook` where id='$reid' ");
		$msg = "<div class=\\'rebox\\'>".addslashes($row['msg'])."</div>\n".$msg;
	}
	$msg = _Replace_Badword($msg);
	$query = "INSERT INTO `sucms_guestbook`(title,mid,uname,uid,msg,ip,dtime,ischeck)
                  VALUES ('$title','{$g_mid}','$uname','$userid','$msg','$ip','$dtime','$needCheck'); ";
	$dsql->ExecuteNoneQuery($query);
	if($needCheck==1)
	{
		ShowMsg('感谢您的留言，我们会尽快回复您！','../plus/gbook.php',0,3000);
		exit();	
	}
	else
	{
		ShowMsg('成功发送一则留言，但需审核后才能显示！','../plus/gbook.php',0,3000);
		exit();
	}
}
//显示所有留言
else
{
	...
	if($page==0) $page=1;
	$tempfile = sucms_ROOT."/templets/".$GLOBALS['cfg_df_style']."/".$GLOBALS['cfg_df_html']."/gbook.html";
...
function viewLeaveWord(){
	if(!empty($_SESSION['sucms_user_name']))
	{
		$uname=$_SESSION['sucms_user_name'];
		$userid =$_SESSION['sucms_user_id'];
	}
	
	$mystr="<div id=\"leaveword\">".
"<form id=\"f_leaveword\"   action=\"/".$GLOBALS['cfg_cmspath']."../plus/gbook.php?action=add\" method=\"post\">".
...
"</tr>".
  ...
  "</tr>".
  "<tr>".
	 	"<td height=\"25\" colspan=\"2\"><input type=\"submit\" click=\"leaveWord()\" value=\"提交留言\" class=\"btn\"/><input name=\"validate\" type=\"text\" id=\"vdcode\" style=\"width:50px;text-transform:uppercase;\" class=\"text\" tabindex=\"3\"/> <img id=\"vdimgck\" src=\"../include/vdimgck.php\" alt=\"看不清？点击更换\"  align=\"absmiddle\"  style=\"cursor:pointer\" onClick=\"this.src=this.src+'?get=' + new Date()\"/><span class=\"red\">*</span></td>".
  "</tr>".
"</table></form>".
"</div>".
...
function leaveWordList($currentPage){
	...
	$sql="select * from `sucms_guestbook` where ischeck='1' ORDER BY id DESC limit $limitstart,$vsize";	
	$cquery = "Select count(*) as dd From `sucms_guestbook` where ischeck='1'";
	...
	else $txt.="<a title='首页' href=\"/".$GLOBALS['cfg_cmspath']."../plus/gbook.php?page=1\">首页</a><a title='前一页' href=\"/".$GLOBALS['cfg_cmspath']."../plus/gbook.php?page=".($currentPage-1)."\">&lt;</a>";
	if($TotalPage==1)
	{
		$txt.="<em>1</em>";
	}else{
	...
	for($i=$x;$i<=$y;$i++)
	{
		$txt.="<a href=\"/".$GLOBALS['cfg_cmspath']."../plus/gbook.php?page=".$i."\">".$i."</a>";
	}	
	}
	if($currentPage==$TotalPage)$txt.="<em class='nolink'>&gt;</em><em class='nolink'>尾页</em>";
	else $txt.="<a href=\"/".$GLOBALS['cfg_cmspath']."../plus/gbook.php?page=".($currentPage+1)."\">&gt;</a> <a href=\"/".$GLOBALS['cfg_cmspath']."../plus/gbook.php?page=".$TotalPage."\">尾页</a>";
	return $txt."</div>";
