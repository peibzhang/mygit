...
$username = $m_user;
$row1=$dsql->GetOne("select username  from sucms_member where username='$username'");
if($row1['username']==$username)
{
	...
	if($username) {
		$dsql->ExecuteNoneQuery("INSERT INTO `sucms_member`(id,username,password,email,regtime,regip,state,gid,points,logincount)
...
{
	$tempfile = sucms_ROOT."/templets/".$GLOBALS['cfg_df_style']."/".$GLOBALS['cfg_df_html']."/reg.html";
...
"<form id=\"f_register\" action=\"/".$GLOBALS['cfg_cmspath']."../plus/reg.php?action=activationsubmit\" method=\"post\">".
...
{
	$mystr="<div id=\"register\">".
"<form id=\"f_Activation\"   action=\"/".$GLOBALS['cfg_cmspath']."../plus/reg.php?action=reg\" method=\"post\">".
"<table align=\"center\" style=\"margin:0 auto\">". 
"<tr>".
